...
def upgrade() -> None:
    ...
    op.execute(
        f"COPY public.standard_job_family FROM '{file_name}' WITH (FORMAT CSV, HEADER TRUE);", execution_options=None
    )
    ...
    op.execute(
        f"""COPY public.job_posting FROM '{file_name}' WITH (FORMAT CSV, HEADER TRUE);""", execution_options=None
    )
